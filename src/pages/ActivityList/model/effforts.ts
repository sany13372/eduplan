...
export const sum = (items: number[]) => items.reduce((acc, item) => acc + item, 0);

export const totalEffort = (efforts: RawEffort[]) => sum(efforts.map((e) => e.minutesAmount));
...
export const normalizeEffortsInMinutes = (efforts: RawEffort[], workKinds: WorkKind[]): EffortLookup => {
  ...
  workKinds
    ...
    .forEach((lessonKind) => {
      lookup[EffortKey.lessonKind(lessonKind.id as string)] = lessonKindEffort(efforts, lessonKind);
    });
...
export const normalizeAndTotalEffortsInMinutes = (efforts: RawEffort[], workKinds: WorkKind[]): EffortLookup => {
  ...
  workKinds.forEach((workKind) => {
    lookup[EffortKey.totalWorkKind(workKind.id as string)] = totalWorkKindEffort(efforts, workKind);
  });
...
export const formatEffort = (effortInMinutes: number, unit: EffortUnit): string => {
  if (effortInMinutes === 0) {
    return unit.unit === 'academic_hours' ? '0' : '00:00';
  }
